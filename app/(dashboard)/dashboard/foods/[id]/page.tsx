...
export default function FoodDetailsPage() {
  ...
  const isEditing = useAppSelector(selectIsEditingFood);
  const editError = useAppSelector(selectEditFoodError);
  ...
  const onSubmit = async (values: FoodFormData) => {
    ...
    if (editFood.fulfilled.match(result)) {
  ...
  if (!food) {
    return (
      <div className="space-y-6">
        <Button variant="ghost" onClick={() => router.push("/dashboard/foods")}>
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              View and edit food information
            </p>
          </div>
        </div>
      </motion.div>
      ...
      {showSuccessMessage && (
        ...
        >
          <p className="text-green-800">Food updated successfully!</p>
        </motion.div>
      )}
                ...
                {form.watch("image") && !imageError && (
                  <div className="relative w-full h-64 rounded-lg overflow-hidden bg-muted">
                    <img
                      ...
                      alt={form.watch("name") || "Food image"}
